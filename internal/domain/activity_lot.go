...
type ActivityLot struct {
	...
	// quantity tracking
	OrigAmount decimal.Decimal `json:"origQty" bson:"origAmount"` // quantity at creation
	Amount     decimal.Decimal `json:"qty"     bson:"Amount"`     // remaining quantity
...
func (a *ActivityLot) Debug() string {
	return fmt.Sprintf("%s-%v-%v-%v", a.Symbol, a.Amount, a.Cost, a.CostValue)
}
...
func NewLotFromActivity(activity Activity) *ActivityLot {
	return &ActivityLot{
		...
		Date:       &activity.Date,
		OrigAmount: activity.RcvAmount,
		Amount:     activity.RcvAmount,
