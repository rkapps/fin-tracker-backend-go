...
func init() {
	...
	migrations.Register(os.Getenv("FINTRACKER_DB_NAME"), 4, "RawItem Schema",
		func(database *mongodb.MongoDatabase) error {
			var err error
			col := mongodb.GetMongoRepository[string, *domain.RawItem](database)
			...
			if err = col.CreateIndexes(context.Background(), []mongo.IndexModel{
				{
					...
					Options: options.Index().SetName("idx_raw_item").SetUnique(true),
				},
			}); err != nil {
				return err
			}
